...
interface Props {
  ...
  /** Hide the heavy per-pane toolbar (used when a universal parent toolbar is present). */
  hideToolbar?: boolean
  /** Custom title label shown when hideToolbar is active. */
  paneTitle?: string
  /** Initial instrument to load into the pane's terminal. */
...
export function ChartPane({
  ...
  const initSym = initialSymbol || symbol
  const loadedSymbolKeyRef = useRef<string | null>(
    initSym?.symbol ? `${initSym.exchange}:${initSym.symbol}` : null
  )
  ...
  useEffect(() => {
    ...
    const callbacks: TerminalCallbacks = {
      ...
      onSymbolLoaded: (view) => {
        if (!aliveRef.current) return
        setSym(view)
        loadedSymbolKeyRef.current = `${view.exchange}:${view.symbol}`
        setQty(1)
  ...
  useEffect(() => {
    ...
    if (loadedSymbolKeyRef.current === key) return
      ...
      {hideToolbar ? (
        ...
        >
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <span
              className={cn('inline-block h-2 w-2 rounded-full', ledClass(wsState))}
              title={`WebSocket ${wsState}`}
            />
            <Button
              variant="ghost"
              size="icon"
              className={cn('h-6 w-6', fullscreen && 'text-primary')}
