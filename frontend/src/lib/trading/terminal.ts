...
const globalDataFeeds = new Map<string, ReturnType<typeof withBarCache>>()
const symbolMetadataCache = new Map<string, Record<string, unknown>>()

function getSharedDataFeed(apiKey: string): ReturnType<typeof withBarCache> {
  let feed = globalDataFeeds.get(apiKey)
  if (!feed) {
    feed = withBarCache(new OpenAlgoDataFeed({ baseUrl: '', apiKey }), { ttlMs: 10 * 60_000 })
    globalDataFeeds.set(apiKey, feed)
  }
  return feed
}
...
export function buildOrderTicket(input: {
  ...
  return {
    ...
    tickSize: sym.tick,
    product:
      DERIVATIVE_EXCHANGES.has(sym.exchange) && (input.product === 'CNC' || !input.product)
        ? 'NRML'
        : (input.product as 'MIS' | 'NRML' | 'CNC'),
    priceType: type,
...
export class TradingTerminal {
    ...
    if (!this.destroyed && sym && data) {
      try {
        ...
        if (
          ...
          return
        const controllerBars = data.bars()
        if (controllerBars && controllerBars.length > 0) {
          const next = [...controllerBars]
        ...
        /* fallback to direct REST below if data.refresh encountered an error */
      }
  ...
  async loadSymbol(pick: SearchRow, opts: { silent?: boolean } = {}): Promise<boolean> {
    ...
    const defaultProduct = DERIVATIVE_EXCHANGES.has(exchange) ? 'NRML' : productOptions[0]
  ...
  async init() {
    ...
    this.cachedBars = getSharedDataFeed(this.apiKey)
