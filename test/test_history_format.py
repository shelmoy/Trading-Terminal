...
def test_invalid_candle_bounds_are_sanitized(broker):
    """Ensure candle bounds satisfy high >= max(open, close, low) and low <= min(open, close)."""
    discrepant_candle = {
        "timestamp": 1_700_000_000,
        "open": 427.0,
        "high": 426.95,  # high lower than open
        "low": 421.05,
        "close": 421.85,
        "volume": -10,  # negative volume
    }
    success, response, status = broker(pd.DataFrame([discrepant_candle]))
    ...
    assert record["volume"] == 0  # clipped


def test_nan_candle_rows_are_dropped(broker):
    """Rows with NaN in open/high/low/close/timestamp are dropped."""
    nan_candle = {
        "timestamp": 1_700_000_000,
        "open": None,
        "high": 100.0,
    ...
    success, response, status = broker(pd.DataFrame([CANDLE, nan_candle]))

    assert success is True
    assert status == 200
    assert len(response["data"]) == 1
    assert response["data"][0]["timestamp"] == CANDLE["timestamp"]
