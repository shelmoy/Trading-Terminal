...
# Fast in-memory cache for option chain responses (3.0s TTL for sub-millisecond deduplication)
_CHAIN_STRUCTURE_CACHE: dict[tuple, tuple[float, bool, dict[str, Any], int]] = {}
_CHAIN_STRUCTURE_CACHE_TTL = 3.0
...
def get_option_chain(
    ...
    """
    # Check in-memory cache for ultra-fast (0ms) option chain returns
    cache_key = (
        str(underlying).upper(),
        str(exchange).upper(),
        str(expiry_date).upper() if expiry_date else "",
        strike_count if strike_count is not None else -1,
        bool(with_quotes),
        bool(with_greeks),
    )
    now = time.monotonic()
    if cache_key in _CHAIN_STRUCTURE_CACHE:
        cached_time, success, response_data, status_code = _CHAIN_STRUCTURE_CACHE[cache_key]
        if now - cached_time < _CHAIN_STRUCTURE_CACHE_TTL:
            return success, response_data, status_code
        else:
            del _CHAIN_STRUCTURE_CACHE[cache_key]
    ...
    try:
        ...
        _CHAIN_STRUCTURE_CACHE[cache_key] = (now, True, response_data, 200)
