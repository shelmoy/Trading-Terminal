...
def get_quotes(
    ...
    if cache_key in _QUOTE_CACHE:
        ...
        if now - cached_time < _QUOTE_CACHE_TTL:
    ...
    # Case 2: Direct internal call with auth_token and broker
        ...
        success, response_data, status_code = get_quotes_with_auth(
            auth_token, feed_token, broker, symbol, exchange
        )
    ...
    if success and status_code == 200:
        _QUOTE_CACHE[cache_key] = (time.monotonic(), success, response_data, status_code)
...
def get_multiquotes_with_auth(
    ...
    Returns:
        ...
        - HTTP status code (int)
    """
    # Check in-memory cache for all symbols (0ms return if all cached)
    now = time.monotonic()
    cached_results = []
    all_cached = True
    for s in symbols:
        ...
        all_cached = False
        break
    if all_cached and cached_results and len(cached_results) == len(symbols):
        return True, {"status": "success", "results": cached_results}, 200
