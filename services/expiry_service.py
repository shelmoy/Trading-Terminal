...
# Ultra-fast in-memory cache for expiry dates (0ms lookup)
_EXPIRY_CACHE: dict[tuple[str, str, str], tuple[float, bool, dict[str, Any], int]] = {}
_EXPIRY_CACHE_TTL = 300.0  # 5 minutes TTL
...
def get_expiry_dates(
    ...
    try:
        ...
        cache_key = (symbol, exchange, instrumenttype)
        now = time.monotonic()
        if cache_key in _EXPIRY_CACHE:
            cached_time, success, response_data, status_code = _EXPIRY_CACHE[cache_key]
            if now - cached_time < _EXPIRY_CACHE_TTL:
                return success, response_data, status_code
            else:
                del _EXPIRY_CACHE[cache_key]
        ...
        _EXPIRY_CACHE[cache_key] = (now, True, response_data, 200)
