...
class KotakWebSocketAdapter(BaseBrokerWebSocketAdapter):
    ...
    def _on_data_received(self, parsed_data):
        ...
        try:
            ...
            with self._lock:
                # Associate internal index token dynamically when data arrives with trading symbol
                mapping_key = (broker_exchange, token)
                ts = parsed_data.get("ts", "")
                if mapping_key not in self._kotak_to_openalgo and ts:
                    name_key = (broker_exchange, ts)
                    if name_key in self._kotak_to_openalgo:
                        self._kotak_to_openalgo[mapping_key] = self._kotak_to_openalgo[name_key]
                        if name_key in self._symbol_modes:
                            self._symbol_modes[mapping_key] = set(self._symbol_modes[name_key])
                        logger.info(
                            f"[KOTAK SFEED] Mapped index token {broker_exchange}:{token} to "
                            f"{self._kotak_to_openalgo[mapping_key]} (index name '{ts}')"
                        )
                elif mapping_key in self._kotak_to_openalgo and ts:
                    name_key = (broker_exchange, ts)
                    if name_key in self._symbol_modes:
                        if mapping_key not in self._symbol_modes:
                            self._symbol_modes[mapping_key] = set()
                        self._symbol_modes[mapping_key].update(self._symbol_modes[name_key])
            ...
            with self._lock:
                if mapping_key in self._kotak_to_openalgo:
                    ...
                    if ts:
                        active_modes.update(self._symbol_modes.get((broker_exchange, ts), set()))
                    for m in (1, 2, 3):
                        if f"{exchange}|{symbol}|{m}" in self.subscriptions:
                            active_modes.add(m)
            ...
            if exchange and symbol:
                from broker.kotak.streaming.kotak_mapping import is_index_exchange
                is_index = is_index_exchange(exchange)

                # Determine effective modes to publish
                modes_to_publish = set(active_modes)
                if is_index:
                    # Indices have no depth; index ticks always carry LTP and OHLC
                    modes_to_publish.add(1)
                    modes_to_publish.add(2)
                else:
                    # Higher modes cover lower modes for multi-client subscribers
                    if 3 in modes_to_publish:
                        modes_to_publish.add(1)
                        modes_to_publish.add(2)
                    elif 2 in modes_to_publish:
                        modes_to_publish.add(1)

                for mode in modes_to_publish:
    ...
    def subscribe_quote(self, exchange, symbol, mode):
        ...
        try:
            from broker.kotak.streaming.kotak_mapping import (
                get_kotak_exchange,
                get_kotak_index_symbol,
                is_index_exchange,
            )
            from database.token_db import get_token
            ...
            if is_index:
                token_str = get_kotak_index_symbol(symbol)
                sub_type = "ifs"
            else:
                token = get_token(symbol, exchange)
                if not token:
            ...
            logger.debug(f"Mapping: {exchange}:{symbol} -> {kotak_exchange}:{token_str} (sub_type={sub_type})")

            # Store mapping and track mode - CRITICAL FOR MULTI-CLIENT SUPPORT
            with self._lock:
                mapping_key = (kotak_exchange, token_str)
                ...
                if is_index:
                    ...
                    if raw_token:
                        raw_key = (kotak_exchange, str(raw_token))
                        self._kotak_to_openalgo[raw_key] = (exchange, symbol)
                        if raw_key not in self._symbol_modes:
                            self._symbol_modes[raw_key] = set()
                        self._symbol_modes[raw_key].add(mode)

                # Ensure all mapping keys pointing to this (exchange, symbol) track this mode
                for k, v in self._kotak_to_openalgo.items():
                    if v == (exchange, symbol):
                        if k not in self._symbol_modes:
                            self._symbol_modes[k] = set()
                        self._symbol_modes[k].add(mode)
            ...
            self._enqueue_subscription(kotak_exchange, token_str, sub_type=sub_type)
            logger.debug(
                f"Queued quote subscription: {exchange}:{symbol} "
                f"(kotak: {kotak_exchange}|{token_str}, sub_type={sub_type})"
            )
            return True
    ...
    def unsubscribe_quote(self, exchange, symbol, mode):
        ...
        try:
            ...
            with self._lock:
                ...
                if mapping_key in self._symbol_modes:
                    ...
                    if not self._symbol_modes[mapping_key]:
                        ...
                        symbol_key = f"{kotak_exchange}|{token_str}"
                ...
                if is_index:
                    ...
                    if raw_token:
                        raw_key = (kotak_exchange, str(raw_token))
                        if raw_key in self._symbol_modes:
                            self._symbol_modes[raw_key].discard(mode)
                            if not self._symbol_modes[raw_key]:
                                self._kotak_to_openalgo.pop(raw_key, None)
                                self._symbol_modes.pop(raw_key, None)

                # Remove mode across all mapping keys for this (exchange, symbol)
                for k, v in list(self._kotak_to_openalgo.items()):
                    if v == (exchange, symbol):
                        if k in self._symbol_modes:
                            self._symbol_modes[k].discard(mode)
            ...
            if should_unsub_broker:
                self._enqueue_subscription(kotak_exchange, token_str, sub_type=sub_type)
                logger.debug(f"Queued broker unsubscribe: {exchange}:{symbol} (sub_type={sub_type})")
    ...
    def subscribe_depth(self, exchange, symbol, mode):
        ...
        try:
            ...
            # Indices do not have depth / order books
            if is_index_exchange(exchange):
                logger.debug(f"Skipping depth subscription for index {exchange}:{symbol}")
                return True
    ...
    def unsubscribe_depth(self, exchange, symbol, mode):
        ...
        try:
            ...
            # Indices do not have depth / order books
            if is_index_exchange(exchange):
                logger.debug(f"Skipping depth unsubscription for index {exchange}:{symbol}")
                return
