...
INDEX_EXCHANGES = {"NSE_INDEX", "BSE_INDEX"}

KOTAK_INDEX_SYMBOL_MAP = {
    "NIFTY": "Nifty 50",
    "NIFTY50": "Nifty 50",
    "NIFTY 50": "Nifty 50",
...
def is_index_exchange(exchange: str) -> bool:
    """Check if the exchange represents an index."""
    if not exchange:
        return False
    return exchange.upper() in INDEX_EXCHANGES or exchange.upper().endswith("_INDEX")


def get_kotak_index_symbol(symbol: str) -> str:
    """Get the Kotak SFeed index name for an index symbol."""
    if not symbol:
        return symbol
    return KOTAK_INDEX_SYMBOL_MAP.get(symbol.upper(), symbol)
