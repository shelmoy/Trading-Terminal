...
def map_order_data(order_data):
    ...
    Parameters:
    - order_data: A dict from broker API with 'data' field containing order list.

    Returns:
    - The modified list of orders with updated symbols and exchanges, or [] if no data.
    """
    if not isinstance(order_data, dict):
        return []

    if order_data.get("stat") == "Not_Ok":
        logger.debug("No order data available (stat != Ok).")
        return []

    data = order_data.get("data")
    if data is None:
        logger.debug("No order data available (data is None).")
        return []
    ...
    for order in data:
        if isinstance(order, dict):
            exchange = map_exchange(order.get("exSeg"))
...
def calculate_order_statistics(order_data):
    ...
    if order_data and isinstance(order_data, list):
        for order in order_data:
            ...
            if order.get("trnsTp") == "B":
                order["trnsTp"] = "BUY"
                total_buy_orders += 1
            elif order.get("trnsTp") == "S":
                order["trnsTp"] = "SELL"
                total_sell_orders += 1

            # Normalize "trigger pending" to "open" for UI compatibility
            if order.get("ordSt") == "trigger pending":
                order["ordSt"] = "open"

            # Count orders based on their status
            if order.get("ordSt") == "complete":
                total_completed_orders += 1
            elif order.get("ordSt") == "open":
                total_open_orders += 1
            elif order.get("ordSt") == "rejected":
...
def transform_order_data(orders):
    ...
    if isinstance(orders, dict):
        if not orders.get("nOrdNo") and not orders.get("trdSym"):
            return []
        orders = [orders]
    ...
    for order in orders:
        if not isinstance(order, dict) or not order:
            continue
        # Skip dummy rows with no order ID and no trading symbol
        if not order.get("nOrdNo") and not order.get("trdSym"):
            continue
...
def map_trade_data(trade_data):
    ...
    Parameters:
    - trade_data: A dict from broker API with 'data' field containing trade list.

    Returns:
    - The modified list of trades with updated symbols and exchanges, or [] if no data.
    """
    if not isinstance(trade_data, dict):
        return []

    if trade_data.get("stat") == "Not_Ok":
        logger.debug("No trade data available (stat != Ok).")
        return []

    data = trade_data.get("data")
    if data is None:
        logger.debug("No trade data available (data is None).")
        return []
    ...
    for order in data:
        if isinstance(order, dict):
            ...
            if order.get("trnsTp") == "B":
                ...
                order["trnsTp"] = "SELL"
    logger.debug(f"Mapped Kotak tradebook: {data}")
    return data
...
def transform_tradebook_data(tradebook_data):
    ...
    if isinstance(tradebook_data, dict):
        if not tradebook_data.get("nOrdNo") and not tradebook_data.get("trdSym") and not tradebook_data.get("flId"):
            return []
    ...
    for trade in tradebook_data:
        if not isinstance(trade, dict) or not trade:
            continue
        # Skip dummy rows with no order ID, symbol, or fill ID
        if not trade.get("nOrdNo") and not trade.get("trdSym") and not trade.get("flId"):
            continue
