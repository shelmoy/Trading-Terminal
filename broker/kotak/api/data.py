...
class BrokerData:
    ...
    def _history_neosymbols(self, symbol: str, exchange: str, segment: str) -> list:
        ...
        # For index symbols, Kotak Neo historical API expects descriptive names
        # (e.g. "nse_cm|Nifty 50") rather than internal numeric tokens (which
        # return dummy zero-price candles), so put descriptive names first.
        if "INDEX" in exchange.upper():
    ...
    def get_history(
        ...
        try:
            ...
            while current_start <= end:
                ...
                for neosymbol in attempts:
                    try:
                        ...
                        continue
                    # Check if result has actual valid non-zero candle data
                    has_valid_prices = any(
                        len(c) >= 5 and (c[1] > 0 or c[4] > 0)
                        for c in (result or [])
                    )
                    if result and has_valid_prices:
                        resolved = neosymbol
                        candles = result
                        break
                    elif result and not has_valid_prices:
                        logger.warning(
                            f"HISTORY API - Candidate {neosymbol} returned {len(result)} candles with zero prices, trying next candidate"
                        )
                        continue
                    elif result == []:
                        # Empty list (no data found for this date range)
                        candles = []
                        break
            ...
            final_df = final_df.dropna(subset=["timestamp", "open", "high", "low", "close"])
            final_df = final_df[(final_df["open"] > 0) & (final_df["close"] > 0)]

            # Enforce chart library invariants: high >= max(open, close, low) and low <= min(open, close)
            final_df["high"] = final_df[["open", "high", "close"]].max(axis=1)
            final_df["low"] = final_df[["open", "low", "close"]].min(axis=1)
            final_df["volume"] = final_df["volume"].clip(lower=0)
