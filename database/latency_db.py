...
class OrderLatency(LatencyBase):
    ...
    def get_latency_stats():
        ...
        try:
            ...
            # Exclude bulk historical candle downloads from order execution statistics
            ...
            ).filter(OrderLatency.order_type != "HISTORY").first()
            ...
            if total_orders > 0:
                total_latencies = [
                    ...
                    .filter(
                        ...
                        OrderLatency.order_type != "HISTORY",
            ...
            broker_agg = (
                ...
                .filter(OrderLatency.broker.isnot(None), OrderLatency.order_type != "HISTORY")
